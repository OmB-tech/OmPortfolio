...
const TerminalAbout = ({ name }) => {
    ...
    return (<div className="font-mono bg-zinc-900 border border-zinc-700 rounded-lg p-4 h-48 flex items-center">
      <div className="text-zinc-300">
        <div className="flex items-center h-[40%]">
          <span className="text-2xl">{displayText}</span>
          <span className={`ml-0.5 inline-block w-2.5 h-5 bg-emerald-400 ${cursorVisible ? "opacity-100" : "opacity-0"} transition-opacity duration-100`}/>
        </div>
      </div>
    </div>);
};
